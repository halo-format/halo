...
**Observed** — one run, `gpt-5.5`, a ~270KB attachment. A live agent is non-deterministic, so treat
...
|---|---|---|---|
| baseline | 186,168 tok | $0.373 | 1 |
| **halo** | **1,043 tok** | **$0.003** | 1 |

≈ **99% less context and cost**, with the **identical** answer (`periapical_xray`; findings support
the crown on #19) — the ~200KB `image_b64` never enters context. <br>\*Cost is computed at the
harness's fixed gpt-4.1 rate, so the absolute figure is approximate for another model; both arms use
the same rate, so the relative reduction holds.
