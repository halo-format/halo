"""The 15 payer tools as OpenAI Agents SDK function tools, for the OpenAI Agents port.

Each tool wraps the same SQL on ext.*/agent.* and the same deterministic engine as
every other port of this agent — only the framing changes (the OpenAI Agents SDK's
``function_tool``). The contract is the swap point: SQL today, real payer feeds later.

The bodies here are plain async functions (so the self-test can await them directly);
``TOOLS`` at the bottom wraps each one with ``function_tool`` — which derives the JSON
schema from the type hints and the description from the docstring. ``function_tool``'s
strict mode rejects an untyped ``list[dict]``, so the one tool that takes a list of
free-form line objects (``payer_record_decision``) is wrapped with ``strict_mode=False``.
...
NOTE: this example deliberately ships **without** a Halo integration — tools return plain
JSON. The Halo host adapter for the OpenAI Agents SDK (which keeps large tool results out
of the model's context and records content-addressed evidence) is wired in separately in
``main.py`` (HALO=1); the on-demand attachment body — narrative/findings the reviewer
reads, plus an ``image_b64`` blob it never reads — is exactly the payload that adapter
encodes.
"""
...
# ── tools (plain async functions; wrapped into FunctionTools at the bottom) ────
...
# ── registration ───────────────────────────────────────────────────────────────
# Wrap each plain function as a FunctionTool. The name is taken from the function name and
# the description from the docstring; the schema is derived from the type hints.
# payer_record_decision takes a list of free-form line objects, which strict mode rejects
# (an untyped object can't forbid additionalProperties), so it opts out of strict_mode.
TOOLS = [
    function_tool(payer_get_agent_provenance),
    function_tool(payer_get_claim),
    function_tool(payer_get_attachment),
    function_tool(payer_get_attachments),
    function_tool(payer_get_member_coverage),
    function_tool(payer_get_benefit_rules),
    function_tool(payer_get_accumulators),
    function_tool(payer_get_claim_history),
    function_tool(payer_check_network),
    function_tool(payer_get_allowed_amount),
    function_tool(payer_lookup_reason_code),
    function_tool(payer_adjudicate_line),
    function_tool(payer_record_decision, strict_mode=False),
    function_tool(payer_request_review),
    function_tool(payer_post_adjudication),
]
