...
// Output serialization: JSON (default) or TOON (compact, in-context). TOON is the "compact format"
// arm of the A/B — it shrinks the payload but it still lands in the model's context (no fetch).
const FORMAT = process.env.MONITORING_FORMAT || "json";
...
const result = (obj: unknown) => {
  // JSON-normalize first (pg returns Dates/Decimals etc.) so TOON sees plain JSON values.
  const text = FORMAT === "toon" ? toonEncode(JSON.parse(JSON.stringify(obj))) : JSON.stringify(obj);
...
async function getIssueDetail(args: Json) {
  ...
  return withClient(async (c) => {
    ...
    // Argument-join: fold repeated lookups of this issue into one growing map (Halo on only).
    if (halo.haloEnabled()) {
      env.map_root = await halo.accumulate(c, SESSION_ID, issueId, env as halo.Envelope, {
        issue_id: issueId,
      });
    }
    return env;
