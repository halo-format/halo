...
// long run is fetchable late, and runs can be replayed. Maps (agent.halo_maps)
// keep the latest root per entity (e.g. an issue id) so repeated calls about the
// same issue fold into one growing map — "argument-join".
// ============================================================================
...
/**
 * Halo can be turned off (MONITORING_HALO=0) to measure the agent against raw
 * tool results — the A/B baseline. On by default.
 */
export function haloEnabled(): boolean {
  return (process.env.MONITORING_HALO ?? "1") !== "0";
}
 ...
 *
 * With Halo OFF, returns the FULL payload inline (summary fields + every heavy
 * part expanded) — i.e. the raw result the model would otherwise have to read in
 * full. This is the no-Halo baseline arm.
 */
export async function encode(
  ...
  if (!haloEnabled()) {
    return { ...(summary as Record<string, unknown>), ...heavy };
  }
