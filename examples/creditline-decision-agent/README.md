...
> an opt-in **Halo** integration used to compare token usage. With `HALO=1` every
> tool result is routed through the Halo host adapter (`halo_format_claude`) so
> large results stay out of the model's context and the agent fetches only the
> fields it needs. See [Token comparison with Halo](#token-comparison-with-halo).
...
run prints a `TOKENS` summary (input / output / cache / total + cost + turns +
per-tool call counts) and writes `runs/<label>.json`.
...
# larger bureau payload (BUREAU_TRADELINES tunes the report size):
BUREAU_TRADELINES=220 scripts/run_token_ab.sh baseline-big
