...
from halo_format import extend as _extend
...
class HaloSession:
    ...
    def ingest(self, tool_name: str, tool_input, value) -> dict:
        """Encode a tool result into the store and return its envelope and map id. Related calls (per
        key_of) fold into one entity map; unrelated calls each get a fresh id m1, m2, ..."""
        map_id = self._key_of(tool_name, tool_input)
        if map_id is None:
            self._synthetic += 1
            map_id = f"m{self._synthetic}"

        prior = self._maps.get(map_id)
        if prior is not None:
            result = _extend(prior["root"], tool_name, value, self.store, alg=self._alg)
        else:
            result = _encode({tool_name: value}, self.store, alg=self._alg)
